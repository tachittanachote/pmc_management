...
$(document).ready(() => {
    ...
    $("#upload-work").on("click", function (e) {
        ...
        if ((product.find(':selected').attr('data-fix') === "no" && !employee.val()) || (product.find(':selected').attr('data-fix') === "no" && employee.val().length <= 0)) {
            employee.addClass("is-invalid")
            employeeFeedback.css('display', 'block')
        }
    ...
    $(employee).on('change', function (e) {
        if (employee.val().length > 0) {
            ...
            employeeFeedback.css('display', 'none')
        }
